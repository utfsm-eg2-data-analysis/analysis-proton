...
void get_hists_zlc(TString tar){
  ...
  TString outf_name = "hists_zlc_"+targ+".root";
  TFile *outf = new TFile(outf_name,"RECREATE");


  TH1F *hist_Rec = new TH1F("hist_Rec", "hist_Rec", 30, 0.0, 0.3);
  
  TH1F *hist_Gen = new TH1F("hist_Gen", "hist_Gen", 30, 0.0, 0.3);
  ...
  for(int k=1; k<=30; k++){
	...
	for(int l=1; l<=25; l++){
	  ...
	  TString tree_f = "/lustre19/expphy/volatile/clas/claseg2/jpgarces/Acceptance_out/zlc/"+targ+"/Tree_" + targ +"_"+ run +"_zlc.root";

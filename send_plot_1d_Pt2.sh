...
#########################################################################################
# ./send_plot_1d_Pt2.sh <tarname> <code 1>                                              #
#     <tarname>= (D,C,F (for Fe),P (for Pb))                                            #
#     <code 1> = (1-25) -> 01-merged,...,25merged                                       #
# for((i=2;i<=25;i++)); do ./send_plot_1d_Pt2.sh F ${i}; done                           #
# EG: ./send_plot_1d_Pt2.sh P 1                                                         #
#########################################################################################
...
OUTDIR=${HOME}/volatile/Acceptance_out/Pt2 # output dir
TMPDIR=${HOME}/volatile/Acceptance_tmp/Pt2 # temp dir to store logs and job scripts
mkdir -p ${OUTDIR} ${TMPDIR} # just in case

# general options for job
proj="eg2a"
track="analysis" # "analysis" or "debug"
time="50" # "240" or "5" minutes
diskspace="1" # "GB"
memusage="500" # "MB"
osname="general"
...
# inputfile=${SIMDIR}/recsisD_01_1-100.root
    ...
    # setting jobname
    jobname="Acceptance_${TARNAME}_Pt2"      #"GST_${TARNAME}_${RN}"
    ...
    echo "    <Input src=\"${HMDIR}/plot_1d_Pt2.C\"  dest=\"plot_1d_Pt2.C\"/>" >> ${jobfile}
    echo "    <Input src=\"${HMDIR}/run_plot_1d_Pt2.sh\"  dest=\"run_plot_1d_Pt2.sh\"/>"      >> ${jobfile}
    # set command
    echo "    <Command><![CDATA["                                                         >> ${jobfile}
    echo "      sed -i \"s|^TRN=|TRN=${TRN}|g\"       run_plot_1d_Pt2.sh"        >> ${jobfile}
    echo "      sed -i \"s|^FSTN=|FSTN=${FSTN}|g\"       run_plot_1d_Pt2.sh"        >> ${jobfile}
    echo "      chmod 755 ./run_plot_1d_Pt2.sh"                                              >> ${jobfile}
    echo "      sh run_plot_1d_Pt2.sh"                                                       >> ${jobfile}
    echo "    ]]></Command>"                                                              >> ${jobfile}
    # set outputs
    echo "    <Output src=\"Tree_${TARNAME}_${RN}_Pt2.root\"    dest=\"${OUTDIR}/Tree_${TARNAME}_${RN}_Pt2.root\" /> "     >> ${jobfile}
    # set logs

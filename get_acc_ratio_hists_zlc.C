void get_acc_ratio_hists_zlc(TString tar){
...
TString targ;
  ...
  TString infile = "acc_hists_zlc_"+targ+".root";
  TFile *hists_f = TFile::Open(infile);

  TString infile_D = "acc_hists_zlc_D.root";
  TFile *hists_f_D = TFile::Open(infile_D);

  TString outf_name = "acc_ratio_hists_zlc_"+targ+".root";
  TFile *outf = new TFile(outf_name,"RECREATE");

  TH1F *hist_acc_ratio = new TH1F("hist_acc_ratio", "hist_acc_ratio", 30, 0.0, 0.3);

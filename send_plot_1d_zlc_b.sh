...
#########################################################################################
# ./send_plot_1d_zlc_b.sh <tarname> <code 1>                                              #
#     <tarname>= (D,C,F (for Fe),P (for Pb))                                            #
#     <code 1> = (1-25) -> 01-merged,...,25merged                                       #
# for((i=2;i<=25;i++)); do ./send_plot_1d_zlc_b.sh F ${i}; done                           #
# EG: ./send_plot_1d_zlc_b.sh P 1                                                         #
#########################################################################################
...
# inputfile=${SIMDIR}/recsisD_01_1-100.root
    ...
    # setting jobname
    jobname="Acceptance_${TARNAME}_zlc_b"      #"GST_${TARNAME}_${RN}"
    ...
    echo "    <Input src=\"${HMDIR}/plot_1d_zlc_b.C\"  dest=\"plot_1d_zlc_b.C\"/>" >> ${jobfile}
    echo "    <Input src=\"${HMDIR}/run_plot_1d_zlc_b.sh\"  dest=\"run_plot_1d_zlc_b.sh\"/>"      >> ${jobfile}
    # set command
    echo "    <Command><![CDATA["                                                         >> ${jobfile}
    echo "      sed -i \"s|^TRN=|TRN=${TRN}|g\"       run_plot_1d_zlc_b.sh"        >> ${jobfile}
    echo "      sed -i \"s|^FSTN=|FSTN=${FSTN}|g\"       run_plot_1d_zlc_b.sh"        >> ${jobfile}
    echo "      chmod 755 ./run_plot_1d_zlc_b.sh"                                              >> ${jobfile}
    echo "      sh run_plot_1d_zlc_b.sh"                                                       >> ${jobfile}
    echo "    ]]></Command>"                                                              >> ${jobfile}
    # set outputs
    echo "    <Output src=\"Tree_${TARNAME}_${RN}_zlc_b.root\"    dest=\"${OUTDIR}/Tree_${TARNAME}_${RN}_zlc_b.root\" /> "     >> ${jobfile}
    # set logs
